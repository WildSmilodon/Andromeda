...
// Adjust mesh density near the smaller sphere and inside the domain
Field[1] = Distance;
Field[1].NodesList = {1, 2, 3, 4, 5, 6, 7}; // Points of the smaller sphere
Field[2] = Threshold;
Field[2].InField = 1;
Field[2].SizeMin = s; // Minimum element size near the smaller sphere
Field[2].SizeMax = 1;    // Default size
Field[2].DistMin = 0.1;  // Distance where SizeMin is applied
Field[2].DistMax = 1;  // Distance where SizeMax is applied
// Background Field = 2;
...
//+
Surface Loop(1) = {120, 118, 126, 128, 124, 122, 116, 114};
//+
Surface Loop(2) = {14, 16, 18, 26, 28, 20, 24, 22};
//+
Volume(1) = {1, 2};
//+
